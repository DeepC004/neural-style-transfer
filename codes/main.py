...
from depth import calcdepthloss
...
# CONTENT_WEIGHT = 8e-6 # 8e-4
# STYLE_WEIGHT = 80000 # 8e-4

# # The layer to use for the content loss.
# CONTENT_LAYER_NAME = "conv1_conv" # "block2_conv2"
LOSS_Layer = "block5_conv2"
# # List of layers to use for the style loss.
# STYLE_LAYER_NAMES = [
# 'conv2_block2_1_bn','conv3_block1_1_bn','conv3_block1_0_conv','conv4_block1_2_conv',
# ]
#'conv2_block1_1_conv'
...
def get_optimizer():
    return tf.keras.optimizers.experimental.Adam(learning_rate=8)
    # return keras.optimizers.Adam(
    #     keras.optimizers.schedules.ExponentialDecay(
    #         initial_learning_rate = 20, decay_steps = 4, decay_rate = 0.98
    #         #initial_learning_rate = 2.0, decay_steps = 376, decay_rate = 0.98
    #     )
    #)

def compute_loss(feature_extractor, combination_image, content_features, style_features, generated_image, content_image, result_height, result_width):
    combination_features = feature_extractor(combination_image)
    depth_loss(generated_image, content_image, result_height, result_width)
...
def compute_content_loss(content_features, combination_features):
    ...
    depth_tensor = model(preprocess_image('depth_diff.jpg', result_height, result_width))
    depth_image=depth_tensor[LOSS_Layer]
    return tf.reduce_sum(tf.square(generated_image - original_image)+ depth_image) / 2
...
def depth_loss(generated_image, content_image, result_height, result_width):
    temp = deprocess_image(generated_image, result_height, result_width)
    keras.preprocessing.image.save_img('temp.png', temp)
    (calcdepthloss('temp.png', content_image))
...
if __name__ == "__main__":
    ...
    #content_image_path = keras.utils.get_file('/Users/adityasomani/Desktop/3-1/DL/Image-Style-Transfer-Using-Convolutional-Neural-Networks/codes/dataset/tennis.jpeg', 'https://www.google.com/url?sa=i&url=https%3A%2F%2Fwww.atptour.com%2Fen%2Fnews%2Fbig-four-laver-cup-2022-preview&psig=AOvVaw0JjPKVytRVRKAXAkeUbR37&ust=1669961258598000&source=images&cd=vfe&ved=0CBAQjRxqFwoTCMie0NDg1_sCFQAAAAAdAAAAABAN')
    ...
    for iter in range(NUM_ITER):
        with tf.GradientTape() as tape:
            t = generated_image
            loss = compute_loss(model, t, content_features, style_features, generated_image, content_image_path, result_height, result_width)
